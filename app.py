...
if st.button("Predict Fraud"):
    ...
    if prediction == 1:
        st.error(f"⚠️ This transaction is **Fraud**! (Probability: {probability*100:.2f}%)")
    else:
        st.success(f"✅ This transaction is **Not Fraud** (Probability: {probability*100:.2f}%)")

#Accuracy Display
st.sidebar.header("📊 Model Info")
st.sidebar.write("Model: RandomForestClassifier")
st.sidebar.write("Accuracy on test data: **~99.9%** (approx, update with your test accuracy)")
